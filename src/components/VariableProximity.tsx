...
const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      if (distance >= radius) {
        // Reset to from weight
        const fromWeight = parseInt(fromFontVariationSettings.split(" ")[1]);
      ...
      const fromWeight = parseInt(fromFontVariationSettings.split(" ")[1]);
      const toWeight = parseInt(toFontVariationSettings.split(" ")[1]);
      const interpolatedWeight = Math.round(fromWeight + (toWeight - fromWeight) * falloffValue);
      
      interpolatedSettingsRef.current[index] = interpolatedWeight.toString();
      letterRef.style.fontWeight = interpolatedWeight.toString();
      
      // Debug logging
      console.log(`Letter ${index}: distance=${distance}, falloff=${falloffValue}, weight=${interpolatedWeight}`);
    });
  });
          ...
          {word.split("").map((letter) => {
            ...
            return (
                    ...
                    style={{
                      ...
                      fontWeight: 900, // Start with the from weight
                    }}
                    aria-hidden="true"
                  >
                {letter}
